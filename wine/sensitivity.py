...
def visualize_feature_importance(importance_stats, feature_names, save_path=None):
    ...
    axes[0, 0].barh(range(len(sorted_features)), sorted_mean_mag, color='skyblue', edgecolor='navy')
    axes[0, 0].set_yticks(range(len(sorted_features)))
    axes[0, 0].set_yticklabels([f.replace('_', ' ').title() for f in sorted_features], fontsize=9)
    axes[0, 0].set_xlabel('Mean Gradient Magnitude', fontsize=11)
    axes[0, 0].set_title('Feature Sensitivity Rankings\n(Higher = More Important)', fontweight='bold', fontsize=12)
    axes[0, 0].grid(axis='x', alpha=0.3)

    # Add values on bars
    for i, v in enumerate(sorted_mean_mag):
        axes[0, 0].text(v + max(sorted_mean_mag) * 0.01, i, f'{v:.4f}',
                        va='center', fontsize=7)

    # 2. Standard deviation of gradients (variability)
    axes[0, 1].barh(range(len(sorted_features)), sorted_std_mag, color='lightcoral', edgecolor='darkred')
    axes[0, 1].set_yticks(range(len(sorted_features)))
    axes[0, 1].set_yticklabels([f.replace('_', ' ').title() for f in sorted_features], fontsize=9)
    axes[0, 1].set_xlabel('Standard Deviation of Gradient Magnitude', fontsize=11)
    axes[0, 1].set_title('Feature Sensitivity Variability\n(Higher = More Variable Impact)', fontweight='bold',
                         fontsize=12)
    axes[0, 1].grid(axis='x', alpha=0.3)

    # 3. Mean gradient (positive/negative influence)
    colors = ['green' if x > 0 else 'red' for x in sorted_mean_grad]
    axes[1, 0].barh(range(len(sorted_features)), sorted_mean_grad, color=colors, alpha=0.7)
    axes[1, 0].set_yticks(range(len(sorted_features)))
    axes[1, 0].set_yticklabels([f.replace('_', ' ').title() for f in sorted_features], fontsize=9)
    axes[1, 0].set_xlabel('Mean Gradient Value', fontsize=11)
    axes[1, 0].set_title('Feature Influence Direction\n(Green=Positive, Red=Negative)', fontweight='bold', fontsize=12)
    axes[1, 0].axvline(x=0, color='black', linestyle='-', alpha=0.5)
    axes[1, 0].grid(axis='x', alpha=0.3)

    # 4. Max gradient magnitude (peak sensitivity)
    axes[1, 1].barh(range(len(sorted_features)), sorted_max_mag, color='gold', edgecolor='orange')
    axes[1, 1].set_yticks(range(len(sorted_features)))
    axes[1, 1].set_yticklabels([f.replace('_', ' ').title() for f in sorted_features], fontsize=9)
    axes[1, 1].set_xlabel('Maximum Gradient Magnitude', fontsize=11)
    axes[1, 1].set_title('Peak Feature Sensitivity\n(Highest Observed Impact)', fontweight='bold', fontsize=12)
    axes[1, 1].grid(axis='x', alpha=0.3)

    plt.suptitle('Wine Quality Neural Network - Feature Sensitivity Analysis',
...
def create_class_comparison_plot(importance_stats, feature_names, save_path=None):
    ...
    for class_id in classes:
        ...
        for feature in feature_names:
            if feature in class_stats[class_id]:
                class_magnitudes.append(class_stats[class_id][feature]['mean_magnitude'])
            else:
    ...
    plt.title('Feature Sensitivity by Wine Class\n(Darker = More Important for Classification)',
...
def print_sensitivity_results(importance_stats, feature_names, top_k=10):
    ...
    sorted_features = sorted(feature_names,
                             key=lambda f: feature_stats[f]['mean_magnitude'],
                             reverse=True)

    print(f"\nTOP {top_k} MOST SENSITIVE FEATURES")
    print("-" * 80)
    print(f"{'Rank':<4} {'Feature Name':<25} {'Mean Mag':<12} {'Std Mag':<12} {'Max Mag':<12}")
    print("-" * 80)

    for i, feature in enumerate(sorted_features[:top_k]):
        stats = feature_stats[feature]
        print(f"{i + 1:<4} {feature.replace('_', ' ').title():<25} "
    ...
    top_3_avg = np.mean([feature_stats[f]['mean_magnitude'] for f in sorted_features[:3]])
    bottom_3_avg = np.mean([feature_stats[f]['mean_magnitude'] for f in sorted_features[-3:]])
...
def run_sensitivity_analysis(model_path=None, scaler_path=None, save_dir=None,
                             show_plots=True, target_class=None):
    """
    ...
    if model_path and os.path.exists(model_path):
        ...
        model, device, X_scaled, y, scaler, feature_names = prepare_for_sensitivity_analysis(
    ...
    gradient_results = compute_feature_gradients(model, device, X_scaled, y, target_class)
...
def compare_feature_distributions(results, feature_names, save_dir=None):
    ...
    for i, feature in enumerate(feature_names):
        ...
        feature_labels.extend([feature.replace('_', ' ').title()] * len(feature_gradients))

    # Create DataFrame for seaborn
    df = pd.DataFrame({
        'Feature': feature_labels,
        'Gradient_Magnitude': gradient_data
    })

    # Create violin plot
    sns.violinplot(data=df, x='Feature', y='Gradient_Magnitude', ax=ax)
    plt.xticks(rotation=45, ha='right')
    plt.title('Distribution of Gradient Magnitudes by Feature\n(Shows variability in feature sensitivity)',
              fontweight='bold', pad=20)
...
def create_feature_correlation_analysis(results, X_data, feature_names, save_dir=None):
    ...
    ax.set_xlabel('Correlation (Feature Value vs Gradient Magnitude)', fontweight='bold')
    ax.set_title('Feature Value-Sensitivity Correlation\n(Blue=Positive, Red=Negative)',
                 fontweight='bold', pad=20)
